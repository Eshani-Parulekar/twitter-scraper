...
output_csv = "ipowatch_articles.csv"
...
# Function to scrape IPOWatch.in for each IPO
def scrape_ipowatch(ipo_name, ipo_date):
    search_url = f"https://www.ipowatch.in/?s={ipo_name.replace(' ', '+')}"
    driver.get(search_url)
    time.sleep(5)  # Allow JavaScript to load

    soup = BeautifulSoup(driver.page_source, "html.parser")
    articles = soup.find_all("div", class_="post-title")  # Adjust if needed

    results = []

    for article in articles:
        try:
            title_elem = article.find("a")  # Article title and link
            ...
            author_elem = article_soup.find("span", class_="post-author")  
            author = author_elem.text.strip() if author_elem else "Unknown"

            # Extract date and convert format
            date_elem = article_soup.find("span", class_="post-date")  
            if date_elem:
...
for ipo in ipo_data:
    ...
    scraped_data.extend(scrape_ipowatch(ipo["Name"], ipo["Listing Date"]))
