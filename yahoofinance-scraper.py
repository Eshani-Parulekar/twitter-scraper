...
output_csv = "yahoo_finance_articles.csv"
...
# Function to scrape Yahoo Finance for each IPO
def scrape_yahoo_finance(ipo_name, ipo_date):
    search_url = f"https://finance.yahoo.com/lookup?s={ipo_name.replace(' ', '+')}"
    driver.get(search_url)
    time.sleep(5)  # Allow JavaScript to load

    soup = BeautifulSoup(driver.page_source, "html.parser")
    articles = soup.find_all("li", class_="js-stream-content")  # Adjust if needed
    ...
    for article in articles:
        try:
            ...
            link = f"https://finance.yahoo.com{link_elem['href']}" if link_elem else None
            ...
            author_elem = article_soup.find("span", class_="caas-author-name")
...
for ipo in ipo_data:
    ...
    scraped_data.extend(scrape_yahoo_finance(ipo["Name"], ipo["Listing Date"]))
